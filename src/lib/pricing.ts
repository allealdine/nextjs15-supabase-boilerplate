...
class PricingService {
  ...
  static initialize() {
    ...
    const prices =
      process.env.NEXT_PUBLIC_TIERS_PRICES?.split(",").map(Number) || [];
    const descriptions =
      process.env.NEXT_PUBLIC_TIERS_DESCRIPTIONS?.split(",") || [];
    const features =
      process.env.NEXT_PUBLIC_TIERS_FEATURES?.split(",").map((f) =>
        f.split("|")
      ) || [];
